...
1. Navigate to `./bot`, and run `npm install` to install the required node modules.
...
4. run `just dev` to build and run the bot in dev mode.
   - Alternatively, you can manually run the dev commands specified in the `justfile` found in each subdirectory.
